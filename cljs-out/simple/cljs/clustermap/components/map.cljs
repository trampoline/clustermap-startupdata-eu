...
(def initial-bounds [[72.0 43.0] [29.0 -14.0]])
...
(defn style-leaflet-path
  ...
  (cond (and selected highlighted) (.setStyle leaflet-path (clj->js {:color "#003399" :weight 2 :opacity 1 :fill true :fillOpacity 0.2}))
        selected                   (.setStyle leaflet-path (clj->js {:color "#003399" :weight 2 :opacity 1 :fill true :fillOpacity 0.2}))
        highlighted                (.setStyle leaflet-path (clj->js {:color "#000000" :weight 2 :opacity 1 :fill false}))
        true                       (.setStyle leaflet-path (clj->js {:color "#003399" :weight 2 :opacity 0 :fill false :fillOpacity 0}))))
  ...
  (when js-boundaryline-index ;; don't try and render paths until we have path metadata !
    (let [paths @paths-atom
          ...
          new-selection-path-keys ;; (->> new-selection-locations vals (apply concat) (map (comp :uk_constituencies :boundarylinecolls)) (apply concat) set)
                                  #{"geocommons_world_countries__uk" "geocommons_world_countries__sp" "geocommons_world_countries__fr" "geocommons_world_countries__be" "geocommons_world_countries__gm" "geocommons_world_countries__fi" "geocommons_world_countries__sw"}
